...
实模式和保护模式可访问的物理内存空间大小不同。实模式下可访问的物理内存空间不超过1MB，而保护模式下全部32条地址线都有效，可寻址最多4GB的物理地址空间。
物理地址是处理器提交到总线上用于访问计算机系统中内存和外设的最终地址，一个计算机系统中只有一个物理地址空间。
逻辑地址是在有地址变换功能的计算机中访问指令给出的地址。
线性地址是逻辑地址到物理地址变换之间的中间层，处理通过段机制控制下形成的地址空间。
...
```
A: 每一个field在struct中所站的位数，即该成员变量占的位数
- 对于如下的代码段，
